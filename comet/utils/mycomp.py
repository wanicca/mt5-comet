...
@click.option(
    "--results_dir", default="/home/pouramini/images", type=str, help=""
)
@click.option(
    "--unicode",
    default="file2",
...
@click.option("--target_dir", default="", type=str, help="")
@click.option("--comp_dir", default="", type=str, help="")
@click.option(
    "--src_df", default="self", type=str, help="Source dataset or dataframe "
)
...
def comp(
    ...
    def hasNone(p):
        return "--none" in p or (p.strip() == "none" or p.strip() == "هیچ یک")

    if confusion:
        label1, label2 = confusion.split("-")
    for i in tqdm(range(N)):
        if (
            targcol[i] == "target_text" or ":input_text" in inpcol[i]
        ) and i == 0:
            continue
        if not pred_file2:
    ...
    df = pd.DataFrame(data, columns=[target_col, "pred_text1", "input_text",],)
    if src_df and not report:
        ...
        assert len(df) == len(
            srcdf
        ), "the number of source dataset with output perdictions must be the same"
        # qdf2 = srcdf.merge(qdf["pred_text1"], left_index=True, right_index = True)
        print("Results were merged with source dataset")
        df = df[["pred_text1"]]
        df = pd.concat(
            [df.reset_index(drop=True), srcdf.reset_index(drop=True)], axis=1
        )
    ...
    df.to_csv(f"{path}/merge_{fid}.tsv", sep="\t", encoding="utf-8")
    df.columns = df.columns.astype(str)
    if confusion:
        pd.DataFrame(confdata).to_csv(
            f"{path}/{label1}_{label2}_confusion.csv", index=False
        )
    if False:
        match = []
        nones = ["none", "هیچ یک"]
        ...
        qdf = df[
            (df["pred_text1"] == df[target_col]) & ~df[target_col].isin(nones)
        ]
        qdf.to_csv(
            "nn_match_" + Path(path).parent.parent.stem + ".tsv", sep="\t"
        )
        q = len(qdf)
        q2 = len(
            df[
        ...
        if not report:
            ...
            print(
                "====== ",
                str(Path(pred_file2).parent.stem)
                + "/"
                + str(Path(pred_file2).stem),
            )
        print("")
    if confusion:
        print(
            f"Ratio of none confusions between {label1} and {label2}: ({ncc} | {ncc/len(confdata):.2f})"
        )
    print("============================================================")
    if report and not confusion:
        ...
        rep = classification_report(
            targcol, predcol, labels=labels, zero_division=1
        )
        print(rep)
        report_dict = classification_report(
            targcol, predcol, output_dict=True, labels=labels, zero_division=1
        )
        ...
        acc_, cr = confmat.report(
            y_test=targcol,
            y_pred=predcol,
            labels=labels,
            title=title,
            image=path + "/image_" + fid + ".png",
        )
        if results_dir:
