    ...
    for d in split_data:
        for rel in atomic_relation_mappings:
            ...
            if len(d[rel])>0: 
                ...
                query = f"{d['event']} {rel_tokens} {placeholder_token}" #Change this line to modify the encoder input
                ...
                for response in d[rel]:
                    atomic_query_responses[split_name][rel][query].append((f"<extra_id_0> {response} <extra_id_1>",response)) #Change this line to modify the decoder input
                #didn't convert ___ to <blank>
                #didn't normalize to lowercase
