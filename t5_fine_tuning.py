...
# + id="PJX4vkjj6wYz" colab_type="code" outputId="83a8a420-48cd-4d49-bc60-2693268481c6" colab={"base_uri": "https://localhost:8080/", "height": 121}
from google.colab import drive
drive.mount('/content/drive')

# + id="1V5cInhu42Wk" colab_type="code" outputId="5501a5f1-fc49-4df7-f7a0-31cc37647337" colab={"base_uri": "https://localhost:8080/", "height": 302}
# !nvidia-smi

# + [markdown] id="epWcPHhJ7v7j" colab_type="text"
# Instal apex if you want to do 16 bit training. You'll probably need to restart the notebook after installing apex

# + id="k1Xy7ZG-7gHt" colab_type="code" colab={}
# # !export CUDA_HOME=/usr/local/cuda-10.1
# # !git clone https://github.com/NVIDIA/apex
# # !pip install -v --no-cache-dir --global-option="--cpp_ext" --global-option="--cuda_ext" ./apex

# + id="SDVQ04fGRb1v" colab_type="code" outputId="11689986-ca27-4ab0-f14d-5ee4f0eba40d" colab={"base_uri": "https://localhost:8080/", "height": 1000}
# !pip install transformers
# !pip install pytorch_lightning

# + [markdown] id="HVxGfmEMCKs_" colab_type="text"
# ## T5 fine-tuning
#
...
# + id="HS8mNXq6bdxq" colab_type="code" outputId="b0a32f10-f2ef-4d49-b433-266e8206040b" colab={"base_uri": "https://localhost:8080/", "height": 84}
import argparse
import glob
import os
import json
...
# + id="IswYuhWaz7QJ" colab_type="code" colab={}
def set_seed(seed):
  random.seed(seed)
  np.random.seed(seed)
...
# + [markdown] id="RKNr7fgzcKpZ" colab_type="text"
# ## Model
#
...
# + id="B7uVNBtXST5X" colab_type="code" colab={}
...
# + id="oh1R5C-GwMqx" colab_type="code" colab={}
...
# + [markdown] id="a4hjvsBJ5Zk5" colab_type="text"
...
# + id="urduopvizqTq" colab_type="code" colab={}
args_dict = dict(
...
# + [markdown] id="vfhlYUUV2NIh" colab_type="text"
# ## IMDB review classification

# + [markdown] id="b3C13iabZvwK" colab_type="text"
# ### Download IMDB Data

# + id="7R0QdcgXuIWW" colab_type="code" colab={}
# !wget https://ai.stanford.edu/~amaas/data/sentiment/aclImdb_v1.tar.gz
# !tar -xvf aclImdb_v1.tar.gz

# + id="ni1cAK7EvXSB" colab_type="code" colab={}
train_pos_files = glob.glob('aclImdb/train/pos/*.txt')
train_neg_files = glob.glob('aclImdb/train/neg/*.txt')

# + id="jEsRn5pa0v8d" colab_type="code" outputId="6977ce56-d0b4-4d9f-8548-22003bb07eaf" colab={"base_uri": "https://localhost:8080/", "height": 34}
len(train_pos_files), len(train_neg_files)

# + [markdown] id="5zgS8KhlaPiA" colab_type="text"
...
# + id="hLvBHcXwzXrk" colab_type="code" colab={}
# !mkdir aclImdb/val aclImdb/val/pos aclImdb/val/neg

# + id="IXZmLZ1pzjiY" colab_type="code" colab={}
...
# + id="5yTS2Jx40UNu" colab_type="code" colab={}
import shutil

# + id="hJnJpkdb0ZKY" colab_type="code" colab={}
for f in val_pos_files:
  shutil.move(f,  'aclImdb/val/pos')
for f in val_neg_files:
  shutil.move(f,  'aclImdb/val/neg')

# + [markdown] id="qdEgCwL7cIyi" colab_type="text"
# ### Prepare Dataset

# + id="McQC1FotigqA" colab_type="code" outputId="f60dbf68-32cf-44e1-9a2f-f9dba38cbbac" colab={"base_uri": "https://localhost:8080/", "height": 186, "referenced_widgets": ["7d8f60bfc0a248e58028b6e8a477a5f7", "72dc1e39b931429883e68c0603797896", "cde60c5e18f04ba792fff8c2ac33f470", "c0c0df12695b4a1eacf8fa4ccc0ac62c", "72ea881ce3f445a9983d858b76dd257b", "d0f0c28a14b242f8990a547ed7f87c04", "f97741534b554be3b5cdccd45c73b317", "1e70a3dc7090487fa883e932bff395cb"]}
tokenizer = T5Tokenizer.from_pretrained('t5-base')

# + id="wthd9SM74RG8" colab_type="code" outputId="52deb6bd-19c4-4071-8bcb-254925d8e4cc" colab={"base_uri": "https://localhost:8080/", "height": 34}
ids_neg = tokenizer.encode('negative </s>')
ids_pos = tokenizer.encode('positive </s>')
len(ids_neg), len(ids_pos)


# + [markdown] id="k5sJkyI3a723" colab_type="text"
...
# + [markdown] id="VEYmYHKGcxEq" colab_type="text"
...
# + id="IIY0GenSb72m" colab_type="code" colab={}
...
# + id="gsnsKY6jemsr" colab_type="code" outputId="98885b84-7f65-4d79-b470-619def772505" colab={"base_uri": "https://localhost:8080/", "height": 34}
dataset = ImdbDataset(tokenizer, 'aclImdb', 'val',  max_len=512)
len(dataset)

# + id="7g1gz05ccAzg" colab_type="code" outputId="b3a263f1-8b22-46bf-9a33-f58c996d684a" colab={"base_uri": "https://localhost:8080/", "height": 70}
data = dataset[28]
print(tokenizer.decode(data['source_ids']))
print(tokenizer.decode(data['target_ids']))

# + [markdown] id="W4cfw8bMcNdA" colab_type="text"
# ### Train

# + id="aTvkv4rzhPjy" colab_type="code" colab={}
# !mkdir -p t5_imdb_sentiment

# + id="r5ngAP4OXFqZ" colab_type="code" colab={}
...
# + [markdown] id="RJt_VqzEAMUg" colab_type="text"
...
# + id="XhjELPOk7-cz" colab_type="code" colab={}
# # !cp -r t5_base_imdb_sentiment drive/My\ Drive/

# + [markdown] id="brPOSAkjNP5t" colab_type="text"
# ### Eval

# + [markdown] id="_7SuVh05lDrJ" colab_type="text"
# For inference we will use the `generate` method with greedy decoding with max length 2.

# + id="25jbT49CVoXN" colab_type="code" colab={}
import textwrap
from tqdm.auto import tqdm
from sklearn import metrics

# + [markdown] id="cyriGR20lSRa" colab_type="text"
# Let's visualize few predictions on test dataset

# + id="wwJ998sMz2Ci" colab_type="code" colab={}
dataset = ImdbDataset(tokenizer, 'aclImdb', 'test',  max_len=512)
loader = DataLoader(dataset, batch_size=32, shuffle=True)

# + id="2LQtN5b90TyW" colab_type="code" colab={}
it = iter(loader)

# + id="TRD03teH0YMe" colab_type="code" outputId="d43041e6-5d7d-49d5-e91a-7530c5d1d6b1" colab={"base_uri": "https://localhost:8080/", "height": 34}
batch = next(it)
batch["source_ids"].shape

# + id="eewDktozk7GN" colab_type="code" colab={}
...
# + id="9vBe0UNw7cHY" colab_type="code" outputId="2f0171ac-8d7d-41db-db31-d57bf72bc205" colab={"base_uri": "https://localhost:8080/", "height": 1000}
for i in range(32):
...
# + [markdown] id="lATfuiHYHq_1" colab_type="text"
# Now predict on all the test dataset

# + id="lvWQGLXhzHtn" colab_type="code" outputId="c0f5490b-2ade-4795-fa3d-1f0f1746e23c" colab={"base_uri": "https://localhost:8080/", "height": 66, "referenced_widgets": ["6aaf51cb9ad44c94b6a174a8768904f7", "51d23e1199274477a69557c74609afb2", "029f74818c6842d7a28af62032418880", "8db144e9144141779a1088c4bc000a99", "210517aede4f4cfab9120fdeb3d8361a", "df9bc2dc2b3c4fee98affdd7f5ca1ef6", "b684a47485af4cb1934d57cbb03a4f57", "942d20b134964d1d895af69938918464"]}
...
# + [markdown] id="ZBxEcXeWGafd" colab_type="text"
# Let's check if the model generates any invalid text

# + id="Y_qylwYGXgwY" colab_type="code" colab={}
for i, out in enumerate(outputs):
  if out not in ['positive', 'negative']:
    print(i, 'detected invalid prediction')

# + [markdown] id="MpU_VkFGIgnw" colab_type="text"
# This great is great! Our model hasn't generated any invalid prediction. Let's calculate accuarcy and other metrics

# + id="EdJcQODoOChP" colab_type="code" outputId="22fc6852-5443-43e4-d87e-5a5266ddffd9" colab={"base_uri": "https://localhost:8080/", "height": 34}
metrics.accuracy_score(targets, outputs)

# + id="YepnSgI5OKti" colab_type="code" outputId="a2914edf-d572-4166-a886-6c0d731835e5" colab={"base_uri": "https://localhost:8080/", "height": 168}
print(metrics.classification_report(targets, outputs))

# + id="UcZqrJELrRVw" colab_type="code" colab={}


# + [markdown] id="Dhqigmiw2hVh" colab_type="text"
...
# + [markdown] id="0B4IhzEgO21B" colab_type="text"
# ### Download and view data

# + id="6eQhtsD65svj" colab_type="code" outputId="a46f0a9a-27bb-4d10-c7a3-b45b3c894526" colab={"base_uri": "https://localhost:8080/", "height": 1000}
# !wget https://www.dropbox.com/s/ikkqxfdbdec3fuj/test.txt
# !wget https://www.dropbox.com/s/1pzkadrvffbqw6o/train.txt
# !wget https://www.dropbox.com/s/2mzialpsgf9k5l3/val.txt

# + id="yVrcVbvx74G5" colab_type="code" colab={}
# !mkdir emotion_data
# !mv *.txt emotion_data

# + id="jOpnh3Y06BGU" colab_type="code" colab={}
...
# + id="r4sDek6T8PXE" colab_type="code" outputId="a061ba43-03d8-4fdc-b715-b6fca8d57388" colab={"base_uri": "https://localhost:8080/", "height": 313}
data = pd.read_csv(train_path, sep=";", header=None, names=['text', 'emotion'],
                               engine="python")
data.emotion.value_counts().plot.bar()

# + id="EaKp3E1T8kkm" colab_type="code" outputId="7b0fa7d2-199e-4e6e-b895-1d216a1be7b8" colab={"base_uri": "https://localhost:8080/", "height": 195}
train.head()

# + id="i-Gt1WyPBL-6" colab_type="code" outputId="5ca664c8-5a05-4e8c-a15b-b66891f3e164" colab={"base_uri": "https://localhost:8080/", "height": 67}
train.count()

# + id="KybpXVl1Die5" colab_type="code" outputId="1319d2b5-c84e-4c95-bae6-3af745326439" colab={"base_uri": "https://localhost:8080/", "height": 186, "referenced_widgets": ["0037bb8409bb4d65ac4ebd956fd1e631", "db528e3117024014b4d281b650901cbd", "350fc08aa59849fc9fd3f3e454583a6c", "be936dd408314d0d90a22f627ca517ca", "99f56e1a8fdb4b2282fa6e17819d044e", "462bd815ddbc4687bcf7695f59919f0c", "40edb7d92c1145ee9e3bb823e4688e16", "f827cd8a6bf846c590913c5ea40e6737"]}
tokenizer = T5Tokenizer.from_pretrained('t5-base')

# + [markdown] id="cANrUEXhO8QY" colab_type="text"
# ### Dataset

# + [markdown] id="8GsMQdqMPCN7" colab_type="text"
...
# + [markdown] id="AKh6m92eKZc4" colab_type="text"
# Lets check how t5 encodes the following labels

# + id="HDnMp5-fDIAc" colab_type="code" outputId="837d1d28-2d17-4ff0-f345-64eed6949dbb" colab={"base_uri": "https://localhost:8080/", "height": 118}
emotions = [ "sadness", "joy", "love", "anger", "fear", "surprise"]
for em in emotions:
  print(len(tokenizer.encode(em)))


# + [markdown] id="i8VIZIWFOwMj" colab_type="text"
# Here also all the labels are encoded as single ids

# + id="8i8QD-3MDrWq" colab_type="code" colab={}
...
# + id="kRz5jyl3FBkv" colab_type="code" outputId="b3587087-efa7-400b-f3f4-ebc958deb33d" colab={"base_uri": "https://localhost:8080/", "height": 34}
dataset = EmotionDataset(tokenizer, 'emotion_data', 'val', 512)
len(dataset)

# + id="jxT6QzUAFQN0" colab_type="code" outputId="68122a3a-bf3e-4125-f768-a6410abed5a9" colab={"base_uri": "https://localhost:8080/", "height": 50}
data = dataset[42]
print(tokenizer.decode(data['source_ids']))
print(tokenizer.decode(data['target_ids']))

# + id="PBVHtdIuFpID" colab_type="code" colab={}


# + [markdown] id="DEWi6c-pGZV9" colab_type="text"
# ### Train

# + [markdown] id="wGrpDJnLPQ0Q" colab_type="text"
...
# + colab_type="code" id="kDep-uIcGYX2" colab={}
# !mkdir -p t5_emotion

# + id="TgNOy7a4LJ9h" colab_type="code" outputId="3945df44-55d0-40d2-d98c-fa196bb9d554" colab={"base_uri": "https://localhost:8080/", "height": 54}
args_dict.update({'data_dir': 'emotion_data', 'output_dir': 't5_emotion', 'num_train_epochs':2})
args = argparse.Namespace(**args_dict)
print(args_dict)

# + colab_type="code" id="at783kr7KvS4" colab={}
checkpoint_callback = pl.callbacks.ModelCheckpoint(
    filepath=args.output_dir, prefix="checkpoint", monitor="val_loss", mode="min", save_top_k=5
)
...
# + colab_type="code" outputId="93cdd40b-310f-458d-e5ae-21debf158a39" id="aAjhiBcrRQ0E" colab={"base_uri": "https://localhost:8080/", "height": 1000}
for i in range(32):
    c = texts[i]
...
# + [markdown] id="iq8M8nbTSJlE" colab_type="text"
# #### Test Metrics

# + id="S-oIXmoCR6kl" colab_type="code" outputId="98bdff55-aa82-45a3-dc13-be0e78e52ea9" colab={"base_uri": "https://localhost:8080/", "height": 66, "referenced_widgets": ["8933ab7f935e4776970ddfe35f5da135", "84eb2bf17a9048fc94b6f47867d1b0ba", "cdd7554792cf4c73922e2f050d1fcaaf", "a32aa193a82f478387c14f384c2c689e", "e4cbd76c110541cbbf1386e299c4d9d6", "da67548f1abc4727965f72b8cb367681", "63b11aa7ee0c4271aedb87ad3e7d23c3", "720b90b3f86c4e5da15447777806e9a7"]}
dataset = EmotionDataset(tokenizer, 'emotion_data', 'test', 512)
loader = DataLoader(dataset, batch_size=32, num_workers=4)
model.model.eval()
outputs = []
...
# + colab_type="code" id="C9CYCGM6SRzb" colab={}
for i, out in enumerate(outputs):
  if out not in emotions:
    print(i, 'detected invalid prediction')

# + colab_type="code" outputId="24a4fe9c-3396-4364-aad3-8da50d456618" id="iE0WX_GbSRzq" colab={"base_uri": "https://localhost:8080/", "height": 34}
metrics.accuracy_score(targets, outputs)

# + colab_type="code" outputId="01a97ad3-3c70-43b6-e6a4-55ea5ccfa010" id="mWkOZ7BASRz5" colab={"base_uri": "https://localhost:8080/", "height": 235}
print(metrics.classification_report(targets, outputs))

# + [markdown] id="W6p9MGb6lWL5" colab_type="text"
# Now lets plot  the confusion matrix and see for which classes our model is getting confused

# + id="9RtgfuzucFeN" colab_type="code" outputId="0dc41da4-f99e-4469-8d0c-f055d4a18a8d" colab={"base_uri": "https://localhost:8080/", "height": 70}
import seaborn as sn
import pandas as pd
import matplotlib.pyplot as plt

# + id="2ioVvq5rcHZE" colab_type="code" colab={}
cm = metrics.confusion_matrix(targets, outputs)

# + id="4rM5XS09SSdm" colab_type="code" outputId="171788f5-4c43-485c-b84a-133ad78e2486" colab={"base_uri": "https://localhost:8080/", "height": 462}
...
# + [markdown] id="QKh_bJxtlhkW" colab_type="text"
...
# + id="16TiclmeX1xE" colab_type="code" colab={}


# + [markdown] id="vZ-YLmJyg64T" colab_type="text"
# ## SWAG
#
...
# + [markdown] id="hOxk-ZoJmamm" colab_type="text"
# ### Dataset

# + id="yeHfgOhThLPj" colab_type="code" colab={}
import csv
...
# + id="3DulV7U5hik7" colab_type="code" outputId="880c611b-d11c-4620-9d75-0bcfa423c1ff" colab={"base_uri": "https://localhost:8080/", "height": 386}
...
# + id="Tllm6irZg8IO" colab_type="code" colab={}
...
# + id="-OXxGvqZjC9L" colab_type="code" colab={}
...
# + id="oKqFMTku3sDC" colab_type="code" outputId="97ce9f8a-4b75-4d95-ba04-fae101f8db82" colab={"base_uri": "https://localhost:8080/", "height": 186, "referenced_widgets": ["78b1b91a08214461b74fb1e143247d1e", "902a509471004d2691d807c4990fccd2", "74ec15497e1743a4af6be12e3bc1487d", "a70b457d9379403f9fac247de68bb8e3", "28f9d9aa0ece4831b0f9e412d8a88f8d", "7640680e1006492da75d873726567fed", "1090e3e017564a2281c60fb53a901c75", "9df2679ba627444e9b76bd2ff0ddc657"]}
tokenizer = T5Tokenizer.from_pretrained('t5-base')

# + id="PIUiU7zSpbb3" colab_type="code" outputId="328b5f15-fe96-43ce-99e9-5d4233a7e97a" colab={"base_uri": "https://localhost:8080/", "height": 50}
dataset = SwagDataset(tokenizer, data_dir='swag_data', type_path='val')
len(dataset)

# + id="zxXGbCzB37HG" colab_type="code" outputId="8fbda79c-7be7-4d5f-8d5f-7b986a1c374b" colab={"base_uri": "https://localhost:8080/", "height": 70}
data = dataset[69]
print(tokenizer.decode(data['source_ids']))
print(tokenizer.decode(data['target_ids']))

# + [markdown] id="aVfmE4O3Ku7H" colab_type="text"
# ### Train

# + colab_type="code" id="DDPxWUY86llx" colab={}
# !mkdir -p t5_swag

# + colab_type="code" outputId="fe4e58ab-6916-45f9-f742-797d87ad1ef4" id="PrWtMjcj6lmA" colab={"base_uri": "https://localhost:8080/", "height": 54}
args_dict.update({'data_dir': 'swag_data', 'output_dir': 't5_swag', 'num_train_epochs': 3})
args = argparse.Namespace(**args_dict)
print(args_dict)

# + colab_type="code" id="2Ojz3THj6lmK" colab={}
checkpoint_callback = pl.callbacks.ModelCheckpoint(
    filepath=args.output_dir, prefix="checkpoint", monitor="val_loss", mode="min", save_top_k=5
)
...
# + colab_type="code" id="Kk0x0Nql6lmQ" colab={}
def get_dataset(tokenizer, type_path, args):
  return SwagDataset(tokenizer=tokenizer, data_dir=args.data_dir, type_path=type_path,  max_len=args.max_seq_length)


# + colab_type="code" outputId="94aa8d13-9d11-4fa9-979f-e3bbf15bb639" id="XDFGzzpQ6lmU" colab={"base_uri": "https://localhost:8080/", "height": 1000, "referenced_widgets": ["5c7427d7db844b9691d30cf2de1efc17", "bb0df1833ee3489da5c2a9c7b1306cc6", "3d2817812b6f475a8c838fd14646469a", "9d0f0c946790477fb8bc8bac64dfd7de", "8254b8062d5e4280bea46f8bc444c5db", "ab5f07ab5c574148a0062eb7f1ce5bcd", "47fdc2009efc443392ecd182996fcca9", "9b705e83fea84cbf912e33d6342be721", "e8e8ea6199df43019930ac7b557c46a5", "0566f29b017f47f399d7579d7929e046", "932309f0a40b46659c0cac7cc37fdc05", "da3665141bd44a24a5b5c9f36d4a9c52", "5c98e3a5b6a6403a936a725f4c30cdd3", "8da2b560fa9348098a2a7f09967d5f5f", "7e37cac227014717987922341f8099fe", "b95f98f98a76434591f90d41b43e39ba"]}
model = T5FineTuner(args)

# + colab_type="code" outputId="57300f1a-14a8-4e26-8dac-9238e34741c0" id="1sQVILFo63Eb" colab={"base_uri": "https://localhost:8080/", "height": 50}
trainer = pl.Trainer(**train_params)

# + id="STkqK5nC64YP" colab_type="code" outputId="cb613d72-009f-44eb-acd8-b9c3dd44b0cb" colab={"base_uri": "https://localhost:8080/", "height": 1000, "referenced_widgets": ["8e79d03deee94b299431330441bd64c8", "510043ffee634f86b89ec3fc060a74ea", "e86c5fbd48ce4215a0df353122183982", "bfc3a5a3cf2e49868053db6f1ef7785d", "361a2f79ed89495894d0b09a709f8f32", "f7e53d55f0234627a3b9f2c90eb8682f", "3584c01b0c5e47dfa373bae29461e94a", "cfd9db6f31474a8189e741bf8fdad6a9", "68705cee3df5458fb5145046337d925c", "4cf1613d58bd450780ac95c994686985", "3ee5f7cf56394175900ebb14ae0b5f9e", "9f054dcf926c45459b7aa728493571a0", "b52599dda9d94c83891d1c42c5f557e0", "a1cf907a3bcc4177b1d5dd9edbf30c20", "82b29ceeb21c417782e9e29a81eb47ea", "886260804ffd4e11bc93fb6e098111ab", "69f6eb1cb0434128961b5d83529813c5", "6723d50588a248d0ad7bb118de8c3fd5", "86d71b8233c14252a897ffa29ea6d9df", "d01c708e22ab423896271fa79860e7c3", "0e8da5995754472fac5fba1f8b30d107", "3dbee77f299f4e14a1698b60d609b8a1", "8c4c9025aaae44148591ae6f8bb37347", "29e2f2f0914e4dea8117844675b42be5", "0cfc8fa73f164b4fa5ddcbc3f115ef9b", "4559bd35b33f4804b968debaaf316463", "e403cc7718bf48f1b95150482e083f02", "f6248a9db7f2466a9ab3a4fbd214f265", "475e5353d31147d3ab156c0e7835684c", "c3f65d683c6e4fe18e31ecc305f8d455", "9b50abad66b44022aa389bc3f312db6b", "762b2941ff3e47d89b6e6ce4350bc058"]}
trainer.fit(model)

# + id="o1ZB_6SK7V-3" colab_type="code" colab={}


# + [markdown] id="AgNV3TMzqSvj" colab_type="text"
# ### Eval

# + colab_type="code" id="gFFOwfXyqc4_" colab={}
import textwrap
from tqdm.auto import tqdm
from sklearn import metrics

# + colab_type="code" id="rsYCq3Lwqc5Y" outputId="51f7bd88-2441-42be-e8f3-adc0337a164c" colab={"base_uri": "https://localhost:8080/", "height": 34}
dataset =  SwagDataset(tokenizer, data_dir='swag_data', type_path='val')
loader = DataLoader(dataset, batch_size=32, num_workers=4)

# + colab_type="code" outputId="81e7d67d-1d15-4dea-a552-695cfe8ef105" id="KHwMBQNjqc5h" colab={"base_uri": "https://localhost:8080/", "height": 66, "referenced_widgets": ["1597779d89464892885045be715890a8", "8a42468ed6b945e8bfce1803f3ea4452", "f87eae824cf1492b9555b78648a9f261", "6cd0d574b5fd43588b8d492674125218", "17b25142ac744ba882e2bbd1f42c1db2", "09185d325ef84c1fad7b07fbd9eeed31", "ba31765789dc46229493674dab21921d", "a9dd88fb73374e108482b80993b998eb"]}
model.model.eval()
outputs = []
targets = []
for batch in tqdm(loader):
...
# + id="ZbTValmYq15r" colab_type="code" colab={}
for i, out in enumerate(outputs):
  if out not in "1234":
    print(i, 'detected invalid prediction')

# + id="jN35n2pas-pF" colab_type="code" outputId="be8a3507-8e66-479d-c41c-dd9cb0603742" colab={"base_uri": "https://localhost:8080/", "height": 34}
metrics.accuracy_score(targets, outputs)

# + [markdown] id="t_WaMutznvGb" colab_type="text"
...
# + id="rFgOHlW_tHPd" colab_type="code" colab={}
